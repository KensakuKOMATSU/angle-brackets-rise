...
const uint8_t PIN_CLAP = 19;      // 手を叩く（ソレノイド）
const uint8_t PIN_MOUTH = 22;     // 口（ソレノイド）
const uint8_t PIN_FLUID = 21;     // 磁性流体（電磁石）
const uint8_t PIN_EYE = 33;       // 目（LED）
...
bool buttonTestMouthPending = false;
unsigned long buttonTestMouthStartAfter = 0;
...
void applyEyeOutput() {
  ...
  M5.dis.drawpix(0, duty > 0 ? dispColor(ledLevel, ledLevel, ledLevel) : dispColor(0, 0, 0));
}
...
void runButtonTest() {
  // テスト: 手40msの完了後に口100ms
  if (startClapPulse(PULSE_CLAP_MS)) {
    startFluidPulse(PULSE_FLUID_MS);
    buttonTestMouthPending = true;
    buttonTestMouthStartAfter = millis() + PULSE_CLAP_MS;
  }
}
...
void setup() {
  ...
  // Serial.println("{\"status\":\"ready\",\"mode\":\"robot\"}");
}
...
void loop() {
  ...
  if (buttonTestMouthPending) {
    const unsigned long now = millis();
    if (now >= buttonTestMouthStartAfter && clapPulseEndAt == 0) {
      if (startMouthPulse(PULSE_MOUTH_MOVE_MS)) {
        buttonTestMouthPending = false;
      }
    }
  }

  const bool isButtonPressed = M5.Btn.isPressed();
